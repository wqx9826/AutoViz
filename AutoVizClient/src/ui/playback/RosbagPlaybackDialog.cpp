...
void RosbagPlaybackDialog::chooseBag()
{
    // 每个 bag 都是一个目录；选择器应从包含多个 bag 的父目录开始，而不是已加载 bag 的内部。
    QDir initialDirectory(m_path->text());
    if (!m_path->text().isEmpty() && initialDirectory.exists()) {
        initialDirectory.cdUp();
    } else {
        // 安装位置由部署者决定，首次使用时不假设特定用户或 rosbag 根目录。
        initialDirectory = QDir(QCoreApplication::applicationDirPath());
    }
