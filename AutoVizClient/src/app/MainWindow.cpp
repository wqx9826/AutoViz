...
QString topicStatusSummary(const autoviz::datacenter::VisualizationSnapshot& snapshot)
{
    ...
    return QStringLiteral("Server Adapter：%1 个通道，在线 %2，等待/超时 %3\n%4")
...
QString compactTopicStatusSummary(const autoviz::datacenter::VisualizationSnapshot& snapshot)
{
    ...
    return QStringLiteral("Adapter：%1 个通道，在线 %2，等待/超时 %3")
        .arg(snapshot.topicStatuses.size())
        .arg(onlineCount)
        .arg(waitingCount);
}
    ...
    , m_pendingAutoMainViewMode(autoviz::render::MainViewMode::TopDownXY)
{
    ...
    Logger::instance().info("AutoViz Client 初始化完成，UI 仅依赖内部模型和远程协议。");
}
...
{
    m_connectionMenu = menuBar()->addMenu(tr("连接"));
    m_connectServerAction = m_connectionMenu->addAction(tr("连接 Server..."));
    m_disconnectServerAction = m_connectionMenu->addAction(tr("断开连接"));
    m_disconnectServerAction->setEnabled(false);
}
...
void MainWindow::setupStatusBar()
{
    ...
    statusBar()->showMessage(tr("AutoViz Client 初始化中"));
}
...
void MainWindow::connectActions()
{
    ...
    connect(m_aboutAction, &QAction::triggered, this, [this]() {
        QMessageBox::about(this,
                           tr("关于 AutoViz"),
                           tr("AutoViz Client\n\n面向规划控制数据的跨平台可视化工具。\n"
                              "Client 只依赖 AutoViz Protocol，不依赖 ROS。"));
    });
}
...
void MainWindow::refreshVisualization()
{
    ...
    if (snapshot.runtimeStatus.inputSource == autoviz::datacenter::VisualizationInputSource::Remote) {
        const QString channelSummary = compactTopicStatusSummary(snapshot);
        statusBar()->showMessage(m_serverConnected
                                     ? QStringLiteral("%1；%2").arg(m_connectionStatus, channelSummary)
                                     : m_connectionStatus);
...
void MainWindow::initializeRemoteDataSource()
{
    ...
    connect(m_remoteSource,
            &autoviz::network::RemoteVisualizationSource::connectionStateChanged,
            this,
            [this](const QString& text, bool connected) {
                m_connectionStatus = text;
                m_serverConnected = connected;
                m_disconnectServerAction->setEnabled(connected);
                statusBar()->showMessage(text);
                Logger::instance().info(text);
            });

    QSettings settings(QStringLiteral("AutoViz"), QStringLiteral("AutoViz"));
    const QString host = settings.value(QStringLiteral("server/host"),
                                        QStringLiteral("127.0.0.1")).toString();
    const quint16 port = static_cast<quint16>(
        settings.value(QStringLiteral("server/port"), 39090).toUInt());
    const bool autoConnect = settings.value(QStringLiteral("server/auto_connect"), true).toBool();
    if (autoConnect) {
        m_remoteSource->connectToServer(host, port, true);
    } else {
        m_connectionStatus = tr("未连接 Server（%1:%2）").arg(host).arg(port);
        statusBar()->showMessage(m_connectionStatus);
    }
}
...
void MainWindow::openConnectionDialog()
{
    ...
    if (m_connectionDialog == nullptr) {
        m_connectionDialog = new autoviz::ui::ConnectionDialog(this);
    }
    m_connectionDialog->setConnection(
        settings.value(QStringLiteral("server/host"), QStringLiteral("127.0.0.1")).toString(),
        static_cast<quint16>(settings.value(QStringLiteral("server/port"), 39090).toUInt()),
        settings.value(QStringLiteral("server/auto_connect"), true).toBool());
...
void MainWindow::updateMainViewOverlay(const autoviz::datacenter::VisualizationSnapshot& snapshot)
{
    ...
    QStringList capabilities{QStringLiteral("XY")};
    if (status.hasVerticalMotionCapability) {
        capabilities << QStringLiteral("T-Z");
    }
    if (status.hasUnderwaterSystemCapability) {
        capabilities << QStringLiteral("水下系统");
    }
    if (status.hasPlatformDiagnosticsCapability) {
        capabilities << QStringLiteral("平台诊断");
    }
    overlayMessage += QStringLiteral("\n能力：%1").arg(capabilities.join(QStringLiteral(" / ")));
    overlayMessage += QStringLiteral("\n主视图：%1（%2）")
