...
运动总览中的“控制指令”严格区分数据来源：cmd 速度、航向、角速度、模式、档位和使能来自
`/chassis_command`，速度/角速度/档位反馈来自 `/chassis_states`，航向反馈来自 `/location`；
Action 期望值只在 Action 详情中显示，不作为 cmd 回退值。控制曲线在航行模式使用航向角
`°`，在爬行模式使用角速度 `°/s`，模式切换时不会把两类物理量拼接到同一曲线段。命令和
反馈曲线分别在对应 topic 序号或消息时间变化时追加，使用各自消息时间映射到同一绝对时间轴；
50 ms UI 刷新只重绘，不生成采样点。
...
Server 的 20 Hz 完整快照或高倍率回放可能在一次 Client 刷新之间经过多个控制状态；控制卡会
从两种来源共用的事件历史中按顺序展示这些切换帧，每帧至少显示 250 ms 并标注“切换”。因此
短暂 mode=0 不只进入事件时间线，也会显示为“无效（切换） / 未使能”，随后回到最新稳定值。
