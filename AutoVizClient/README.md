...
- “详细信息 -> 底盘 -> 航向驱动器”固定按左尾推、右尾推、左前垂推、右前垂推、后垂推分五行
  显示母线电流 A、控制器温度 °C、目标 rpm 和实际 rpm。Client 优先使用 Protocol 2.8 的
  `ChassisState.thruster_motor=20`；旧 Server 或旧 bag 缺失它时回退 field 14 的两组尾推，三组
  垂推严格显示 `--`，不会因数据源改变字段、单位或顺序。
- Protocol 2.7 的“感知信息 -> 最终融合目标”显示 task ID、水雷统计数、目标 ID/分类、参考点
  XY、保守半径和渔网边界状态；参考点不是几何中心。实时 TCP 与本地 bag 统一使用
  `FinalTargetSet`。旧或截断的 FinalTarget CDR 布局仅跳过该 Topic，不影响同一 bag 的其他已支持通道；
  被跳过帧不会替换上一有效目标集，也不会刷新该 Topic 的消息计数或新鲜度。
- “详细信息 -> TaskParams”采用非对称两栏：左侧固定为基础任务，右侧从上到下紧凑排列爬行
  遥控、航行遥控、推进器调试和配电通路。短小的“爬行遥控”不再被拉伸至基础任务高度，避免
  大块无效留白；所有字段的 optional 缺失显示规则不变。

本地回放按 bag 消息顺序处理控制状态和路径。进入 mode 10/11 时立即清除全局/局部路径，
退出后只显示新到达的路径；暂停、调速和 seek 不会让进度位置领先已应用的控制状态。
“详细信息 -> 控制时序”显示三条控制来源的当前值、接收/记录时间、序号、年龄、过期状态和
Goal UUID；不保存或显示派生的状态切换事件历史。其中“控制状态时间关联”表按数据来源、当前值
和 Goal UUID 的长文本优先分配宽度，序号、刷新年龄
等短字段使用紧凑固定宽度；窄窗口由表格自身横向滚动，不覆盖单元格内容。
Server 的 20 Hz 完整快照或高倍率回放可能在一次 Client 刷新之间经过多个控制状态；控制时序、
运动总览和控制指令均只显示当前完整快照，不为短暂 mode=0 增加 UI 补偿队列、停留计时或
“无效（切换）”展示状态。
