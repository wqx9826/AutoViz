...
- `AutoVizProto/`：根仓库内普通源码目录，是 schema、版本和 FrameCodec 的唯一维护处；只依赖
  C++17、protobuf，提供 `AutoVizProto::AutoVizProto`。
...
- Client 和 Server 必须用 `add_subdirectory()` 消费根目录 `AutoVizProto/` 源码，不得
  `find_package(AutoVizProto)` 或恢复预编译 SDK 流程。`AUTOVIZ_THIRD_PARTY_DIR` 仅用于
  protobuf、Qt 等依赖。
...
Client/Server 直接从同一根仓库编译 Protocol：
...
Server 须在 `AutoVizServer` 目录 source ROS2 和 robot_ws 后使用 `colcon build`；从仓库根的
`AutoVizProto` 编译协议源码。`third_party/COLCON_IGNORE` 防止本地依赖被 colcon 误识别为
workspace 包；协议 GTest 归 AutoVizProto。
