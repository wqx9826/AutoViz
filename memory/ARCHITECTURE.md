...
`ChassisStates` CDR 严格支持旧无尾推/旧含尾推、当前无尾推/含两组尾推及当前五电机的
277/341/325/389/485 字节布局，按固定消息长度选择，最多仅接受 7 字节全零尾部填充。485 布局按
消息顺序在两组尾推后读取左前、右前、后垂推，并同时输出 field 14 的两组旧尾推与 field 20 的
五组通用推进器反馈。旧布局缺失的电机记录保持 optional 缺失，UI 固定显示 `--`，禁止错位解析或
伪造零值。航向控制器诊断仍结构性读取以维持旧 CDR 对齐，但不进入 Client 内部模型或 UI。
